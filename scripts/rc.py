...
def create_ts_user(ts,userinfo):
    ...
    if cmd != 0:
        logger.info("Failed to create TimeSketch user, exited qith status code %d"%cmd)
...
def os_server(args):
    ...
    if args[0].lower() == "stop":
        ...
        if cmd != 0:
            logger.info("Failed to shutdown server, exited qith status code %d"%cmd)
        ...
        if cmd != 0:
            logger.info("Failed to restart server, exited qith status code %d"%cmd)
    else:
...
def os_service(args):
    ...
    if command == "stop":
        for service in service_list_array:
            ...
            if cmd != 0:
                logger.info("Failed to stop %s, exited qith status code %d"%(service, cmd))
        ...
        for service in service_list_array:
            ...
            if cmd != 0:
                logger.info("Failed to start/restart %s, exited qith status code %d"%(service, cmd))
    else:
        logger.warning("Arguments passed: {}".format(args))
        logger.warning("ERROR: Unable to parse Operating System command. Exiting")
        exit(1)
...
def process_cdqr(cdqr,args):
    ...
    if cmd != 0:
        logger.info("Failed process CDQR, exited qith status code %d"%cmd)
...
def mv_local(args):
    ...
    if cmd != 0:
        logger.info("Failed to move file, exited qith status code %d"%cmd)
